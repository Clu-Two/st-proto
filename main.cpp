...
// Loading all the variables here is abhorrent and will be fixed when I implement the database. Gross.
...
// market[] market_data{ steel, radon, graphene, fcells, kryptonite };
market market_data[5] = { steel, radon, graphene, fcells, kryptonite };

//marketO[market::names::STEEL] = &steel;
//marketO[market::names::RADON] = &radon;
//marketO[market::names::GRAPHENE] = &graphene;
//marketO[market::names::FCELLS] = &fcells;
//marketO[market::names::KRYPTONITE] = &kryptonite;
...
while (window.isOpen())
{
    ...
    // Clocks
    if (active.at(indexO.selectedShip).travel_delta.getElapsedTime().asSeconds() >= 0.1f)
    {
        // ship.update(current);
        ...
        //active.at(indexO.selectedShip).update();
            active.at(indexO.selectedShip).travel_delta.restart();
    }
    deltaClock.restart();
